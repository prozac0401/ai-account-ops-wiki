...
2026-04-25~2026-04-26 Cursor 팀 대시보드에서 기존 `Unpaid Admin` 계정이 `Admin` 으로 표시되고, 역할 드롭다운에서 `Unpaid Admin` 선택지가 보이지 않는 현상이 있었다.
...
2026-09-07 편집에서는 이 해결 상태를 보존했다. [[issues/Cursor_청구_크레딧_분쟁|6–7월 청구·크레딧·팀 접근 분쟁]]은 별도 사건이며 이 복구 기록으로 함께 해결 처리하지 않는다. 최신 역할·좌석 유형은 [[services/Cursor|Cursor 운영]]에서 확인한다.
