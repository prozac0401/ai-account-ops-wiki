...
Cursor Teams self-serve 과금은 ChatGPT Business annual처럼 monthly true-up으로 baseline을 다시 맞추는 구조로 보지 않는다. 운영상으로는 사용자를 추가하거나 제거할 때 좌석 수가 바로 조정되고, 남은 기간만큼 prorated 비용 또는 credit이 반영되는 구조로 본다.
...
4. 사용자를 제거하면 unused 기간이 credit으로 돌아올 수 있다.
5. 단, 이미 credit을 사용한 멤버를 제거하면 해당 seat가 결제 주기 끝까지 점유된 것으로 처리될 수 있다.
