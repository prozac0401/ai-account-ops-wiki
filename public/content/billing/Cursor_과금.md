...
Team 계정에서는 좌석 구독료와 모델 사용량을 나누어 본다. 현재 Cursor Team 문서와 가격 안내에서는 Team 좌석에 included usage가 붙는 구조로 설명된다. 다만 included usage 금액, 지역 세금, 프로모션, 플랜명은 바뀔 수 있으므로 실제 구매 전 Billing 화면과 공식 가격 페이지를 다시 확인한다.
...
Cursor Pricing Policy는 true-up을 Active Users 기준으로 설명한다. True Up Period 마지막 30일 동안 실제 Active Users가 기존 Core License seat count를 넘으면, 초과 Core License seat에 대해 남은 Service Term 기간의 Core License Fee와 Precommitted Usage Fee가 invoice로 발행될 수 있다.

예를 들어 20석으로 시작했는데 true-up 기준일의 마지막 30일 Active Users가 28명이고 남은 기간이 8개월이면, 단순 좌석료만 보면 다음처럼 설명할 수 있다.

```text
초과 active users: 28 - 20 = 8 seats
좌석료 예시: 8 seats x $40 x 8 months = $2,560
추가 가능 항목: Precommitted Usage Fee, On-demand Usage
```

같은 팀 안에서 좌석을 다른 사용자에게 재배정하는 것은, 30일 True Up Period 안의 Active Users가 구매 좌석 수를 넘지 않으면 추가 비용 없이 처리될 수 있다. 따라서 교육 운영자는 “초대장을 몇 번 보냈는가”보다 “해당 기간 실제 active users가 몇 명이었는가”를 기록해야 한다.
