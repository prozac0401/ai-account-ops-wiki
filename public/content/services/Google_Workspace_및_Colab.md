...
이 내용은 공개 문서에서 확인한 설명이 아니라, Google Workspace 지원 담당자에게 문의한 뒤 신규 결제 계정의 수량 확대와 관련된 내부 정책성 안내로 파악한 것이다. 당시 안내는 목표 총 사용자 수에 해당하는 월 구독액의 50% 수준을 수동 결제해야 할 수 있고, 이 금액은 향후 인보이스에 적용되는 크레딧으로 남는다는 취지였다.

따라서 이 상황은 Colab만의 고정 규칙이라기보다, Google Workspace 신규 결제 계정의 수량 제한·신용 검증·수동 결제 흐름과 연결해 보는 것이 좋다. 다만 공개 문서로 확인된 보편 규칙은 아니므로 Colab 자체의 보편 규칙이라고 단정하지 않는다.
...
<p class="wiki-footnote" id="abbr-note-1"><a class="wiki-footnote-link" href="#abbr-ref-1">[1]</a> CSV(Comma-Separated Values): 쉼표로 항목을 구분하는 표 형식 파일이다. 대량 할당 명단을 올릴 때 자주 쓴다.</p>
<p class="wiki-footnote" id="abbr-note-2"><a class="wiki-footnote-link" href="#abbr-ref-2">[2]</a> OU(Organizational Unit): Google Workspace에서 사용자를 묶는 조직 단위다.</p>
</div>
...
- Google Workspace Flexible Plan: https://knowledge.workspace.google.com/admin/billing/flexible-plan
