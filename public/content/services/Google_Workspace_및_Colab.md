...
사용자가 Colab Pro/Pro+를 쓰려면 해당 서비스 라이선스가 필요하다. 라이선스는 개별 사용자, CSV(Comma-Separated Values), OU(Organizational Unit) 또는 액세스 그룹 기준으로 할당할 수 있다.
...
| 약어 | 전체 단어 | 설명 |
|---|---|---|
| CSV | Comma-Separated Values | 쉼표로 항목을 구분하는 표 형식 파일이다. 대량 할당 명단을 올릴 때 자주 쓴다. |
| OU | Organizational Unit | Google Workspace에서 사용자를 묶는 조직 단위다. |
