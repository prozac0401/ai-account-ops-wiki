...
프로젝트 경험상 Google 계정은 로그인 위험 판단이 매우 보수적이었다. 특히 새 PC, 새 강의장 네트워크, 짧은 시간의 반복 로그인, 여러 계정의 재활성 작업이 겹치면 같은 사람이 여러 계정을 계속 통과시키기 어려웠다. Workspace 계정 활성화 또는 재활성 흐름에서는 1인당 1계정 정도만 안정적으로 처리되는 패턴이 있었다.
