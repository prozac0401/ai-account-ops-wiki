...
## 수강생 Gmail 계정을 ChatGPT와 Cursor에 등록해도 되나?
...
## Cursor도 ChatGPT처럼 개인정보 고지와 동의가 필요한가?
...
## 개인정보보호법 제15조, 제17조, 제18조, 제28조의8은 왜 나왔나?
...
## OpenAI와 Claude가 제28조의8 수준을 충족할 수 있나?
...
## Cursor의 리텐션 정책은 어디에 있나?

2026-09-07 공식 재확인 기준 [Privacy 설정](https://cursor.com/help/security-and-privacy/privacy), [데이터 사용·예외](https://cursor.com/data-use), [계정 삭제](https://cursor.com/help/account-and-billing/delete-account)를 나누어 읽는다. 과거 Security 페이지의 섹션 이름만으로 최신 정책을 찾지 않는다.

계정 삭제 도움말은 관련 데이터의 30일 내 제거를 안내하지만 PC의 로컬 파일은 남는다. BYOK·별도 보존 모델·정책 위반 조사에는 데이터 보존 예외가 있을 수 있다. [[services/Cursor|Cursor Privacy Mode]]와 [[compliance/데이터_리텐션_삭제|삭제 점검]]에서 적용 범위를 확인한다.

## Colab Pro를 처음에는 2개밖에 못 산 것처럼 보였다. 이게 공식 규칙인가?

공식 규칙으로 단정하지 않는다.

공식 Colab 설정 문서에는 필요한 수만큼 라이선스를 구매할 수 있다는 취지의 설명이 있다. 따라서 운영 문서에는 “결제 계정 상태 또는 신규 구독 제한으로 보인 실제 사례”로 기록한다.

## Colab Pro는 왜 연간계약만 가능했나?

공식 문서에는 온라인 직접 구매 시 Annual/Fixed-Term Plan을 선택해야 한다고 안내되어 있다. Flexible Plan이 필요하면 Google 영업 담당자 또는 Cloud 파트너 경로를 확인한다.

## 수량을 늘리려면 월 구독액 절반을 선결제하라고 나온 이유는?
...
## 보안이 비교적 자유로운 교육장인데도 보안팀 문의가 필요한가?
...
## Cursor에서 기존 Unpaid Admin이 Admin으로 보이면 과금인가?

무조건 과금으로 보지 않는다.

2026-04-25~2026-04-26 Cursor 팀 대시보드에서 기존 `Unpaid Admin` 계정이 `Admin` 으로 표시되고, 역할 드롭다운에서 `Unpaid Admin` 선택지가 사라지는 현상이 있었다. 2026-04-28 표기와 선택지가 복구된 것을 확인했다.
...
## 6–7월 Cursor 청구 분쟁도 해결되었나?

4월 표시 오류와 별개다. [[issues/Cursor_청구_크레딧_분쟁|청구·크레딧·팀 접근 사례]]의 최종 정산·환급·복구는 U02 원문이 필요하다.

## Colab CU를 못 읽으면 0으로 기록하나?

아니다. [[services/Colab_CU와_런타임_운영|CU 대장]]에 실패 상태와 마지막 성공 시각을 남긴다. 라이선스·결제 프로필 회수 후 CU는 전후 증거 없이 소멸·유지로 단정하지 않는다.

## API 예산 알림만 켜면 호출이 차단되나?

아니다. 현재 [[services/OpenAI_API|API 한도]]는 spend alert와 별도 hard spend limit를 구분한다. 429도 지출·잔액·usage tier·rate limit를 나누어 확인한다.
