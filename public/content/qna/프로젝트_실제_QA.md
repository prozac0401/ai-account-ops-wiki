...
이 문서는 본 프로젝트에서 실제로 나온 질문을 운영 문서 형태로 정리한 것이다.

공식 매뉴얼에는 잘 나오지 않는 “실제로 하다 보니 막힌 부분”을 남기는 데 목적이 있다.

## Q1. 교육생 Gmail 계정을 ChatGPT와 Cursor에 등록해도 되나?
...
| 조문 | 쉬운 설명 | 이 프로젝트에서의 의미 |
|---|---|---|
| 제15조 | 개인정보 수집·이용 | 이메일, 이름을 받는 근거와 고지 항목 |
| 제17조 | 제3자 제공 | 외부 SaaS 제공자에게 정보가 넘어가는 구조 검토 |
...
## Q6. ChatGPT Business에서 교육생을 제거하면 이메일도 삭제되나?

멤버 제거와 계정 삭제는 다르다.

교육생이 개인 OpenAI 계정을 사용했다면, 워크스페이스에서 제거해도 개인 계정 자체가 사라지는 것은 아니다. 이메일 주소 삭제나 계정 삭제는 OpenAI 계정 삭제 정책과 사용자의 직접 조치가 필요할 수 있다.
...
공식 Colab 설정 문서에는 필요한 수만큼 라이선스를 구매할 수 있다는 취지의 설명이 있다. 따라서 프로젝트에서는 “결제 계정 상태 또는 신규 구독 제한으로 보인 실제 사례”로 기록한다.
...
Google Workspace Flexible Plan 공식 문서에는 신규 계정이 가입 후 90일 이내에는 사용자 수 제한을 받을 수 있고, 더 늘리려면 목표 총 사용자 수의 50%에 해당하는 수동 결제를 해야 한다는 설명이 있다.

프로젝트 사례는 이 규칙과 연결해 기록한다. 단, Colab 자체의 보편 규칙이라고 단정하지 않는다.
...
## Q13. OpenAI API 키를 교육생에게 직접 줘도 되나?

가능은 하지만 위험하다.

권장 방식은 교육용 프로젝트를 만들고, 예산을 걸고, 서버 쪽에서 키를 보관하는 것이다. 직접 키 발급이 필요하면 교육 종료 후 삭제와 GitHub 노출 방지를 반드시 안내한다.

## Q14. 교육생이 동의하지 않으면 어떻게 하나?
