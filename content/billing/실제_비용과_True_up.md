...
아래 종합 단가 표와 과거 true-up 설명은 **2026-04-25 기준 기록**을 보존한 것으로, 2026-09-07의 전체 현행 견적표가 아니다. 현재 재확인한 [[billing/ChatGPT_OpenAI_API_과금|ChatGPT 좌석·크레딧·API 한도]], [[billing/Cursor_과금|Cursor 좌석·사용량]], [[billing/Claude_과금|Claude 좌석 감소]]를 먼저 읽는다. 최종 금액은 세금·통화·계약과 구매 화면을 대조한다.

## 실제 부과 단가 빠른 표

| 서비스 | 2026-04-25 기록 단가 | 당시 설명·주의 | 과금 성격 |
|---|---:|---|---|
...
예시는 30일 주기, 추가 좌석 사용 20일, 세금·크레딧 없음으로 계산한다. 실제 일수와 청구 시점은 계약에 따른다. 사용월 비용과 다음 주기 청구를 혼합하지 않는다.

```text
예시: ChatGPT Business 월간 플랜

사용월 기본 10석: 10 seats x $25 = $250
추가 3석: 3 seats x $25 x 20/30 days = $50
사용월 귀속 비용: $250 + $50 = $300
다음 주기 기본 13석: 13 seats x $25 = $325
조건부 합산 청구: 다음 기본료 $325 + 아직 청구하지 않은 조정 $50 = $375

현재 즉시 일할 청구로 $50을 이미 냈다면 다음 기본료는 $325다.
```
...
이 절의 baseline 설명은 과거 계산 맥락이다. 현재 신규 좌석 구매는 즉시 일할 청구되므로 "다음 인보이스 $375"를 일반 규칙으로 쓰지 않는다. [2026-09-07 재확인(P26)](https://help.openai.com/en/articles/8792536-managing-billing-and-seats-in-chatgpt-business)
...
과거 연간 사례 = 월별 true-up 정산 주기(현재 계약 적용 여부 별도 확인)
```

### ChatGPT Business annual 예시

**과거 산식 보존:** 다음 월별 true-up 설명은 2026-04-25 기록이다. 2026-09-07 현재 도움말은 추가 좌석 구매의 즉시 청구를 안내하므로, 정산 주기를 현행 신규 플랜에 그대로 적용하지 않는다(U09).
