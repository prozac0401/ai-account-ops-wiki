...
| `STYLE_GUIDE.md` | 문체, 구조, 각주, 출처 표기 기준을 정리한 작성 지침입니다. 위키 화면에는 넣지 않습니다. |
...
약어처럼 본문 흐름을 길게 끊지 않고 설명해야 하는 단어는 나무위키처럼 본문에는 `[1]` 링크만 달고, 문서 하단에 설명을 모읍니다.

본문에서는 설명이 필요한 단어 바로 뒤에 아래 형식을 붙입니다.

```md
DLP(Data Loss Prevention) <sup class="wiki-footnote-ref"><a class="wiki-footnote-link" id="abbr-ref-1" href="#abbr-note-1">[1]</a></sup>
```

문서 맨 아래에는 별도 제목 없이 아래처럼 번호와 설명을 적습니다.
...
- 번호는 문서마다 `1`부터 다시 시작합니다.
- 본문 단어와 `<sup class="wiki-footnote-ref">` 사이에는 공백을 1칸만 둡니다.
- 본문의 `abbr-ref-1`과 하단의 `href="#abbr-ref-1"` 번호가 같아야 합니다.
- 본문의 `href="#abbr-note-1"`과 하단의 `id="abbr-note-1"` 번호가 같아야 합니다.
- 같은 약어가 여러 번 나오면 보통 첫 번째로 중요한 위치에만 각주를 붙입니다.
- 여러 각주를 넣을 때는 본문에 처음 등장하는 순서대로 번호를 붙입니다.
- 하단 각주는 표로 만들지 않고 `[번호] 설명` 형태로 적습니다. `[번호]` 뒤에 하이픈은 넣지 않습니다.
- `## 약어 각주` 같은 별도 제목은 만들지 않습니다. 목차에 잡히지 않는 꼬리말처럼 문서 맨 아래에 붙입니다.
- 실제 위키 문서에서는 각주 HTML을 코드 블록 안에 넣지 않습니다. 코드 블록 안에 넣으면 링크가 아니라 글자 그대로 보입니다.
...
```bash
git status
git add README.md Help.md STYLE_GUIDE.md public/content src/main.js
...
문체, 제목, 각주, 출처 기준은 [STYLE_GUIDE.md](STYLE_GUIDE.md)를 함께 확인합니다.
